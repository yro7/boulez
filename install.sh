...
get_latest_version() {
    ...
    API_RESPONSE=$(curl -sS "https://api.github.com/repos/yro7/boulez/releases")
    if [ $? -ne 0 ]; then
...
extract_and_install() {
    ...
    mv "${tmp_dir}/boulez${extension}" "$bin_dir/$INSTALL_NAME${extension}"
    rm -rf "$tmp_dir"
...
main() {
    ...
    RELEASE_URL="https://github.com/yro7/boulez/releases/download/v${VERSION}"
    ARCHIVE_NAME="boulez_${VERSION}_${PLATFORM}_${ARCHITECTURE}${ARCHIVE_EXT}"
