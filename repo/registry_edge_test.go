...
func TestRegistry_TouchOnMissingFileDoesNotCreateFile(t *testing.T) {
	r := newTestRegistry(t)
	require.NoError(t, r.Touch(t.TempDir()))
	_, err := os.Stat(r.path)
...
func TestRegistry_AddAfterCorruptFile(t *testing.T) {
	...
	dir := t.TempDir()
	require.NoError(t, r.Add(dir), "Add must overwrite the corrupt file with valid JSON")
...
// TestRegistry_TouchUnknownDoesNotCreateFile proves touching an unknown path
...
func TestRegistry_TouchUnknownDoesNotCreateHeadEntry(t *testing.T) {
	...
	require.NoError(t, r.Touch(t.TempDir())) // unknown
	paths, err := r.List()
	require.NoError(t, err)
	assert.Equal(t, []string{a}, paths, "unknown touch must not add an entry")
}
...
// must be symmetric across Add/Remove/Touch/Contains).
func TestRegistry_RemoveRelativeMatchesAbsoluteAdd(t *testing.T) {
	...
	paths, err := r.List()
	require.NoError(t, err)
	assert.Empty(t, paths, "relative remove must match absolute add")
	assert.False(t, r.Contains(abs))
}
