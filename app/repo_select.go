...
// per repo). The repo selector starts with the host-scoped list and is narrowed
// when this result lands, so local users see no flicker while remote users see
// the inaccessible entries drop once probed.
...
// m.pendingHost's executor (local or ssh). The offered repos are scoped to the
// chosen host: a remote host only sees repos registered against it, so a
...
func (m *home) openRepoSelector(promptFlow bool) tea.Cmd {
	...
	if m.repoRegistry != nil {
		repos, _ = m.repoRegistry.ListByHost(h.Name())
	}
	m.repoSelector = overlay.NewRepoSelector(repos)
	m.repoSelectPrompt = promptFlow
	m.state = stateRepoSelect

	// Filter the host-scoped repos against the chosen host's executor: a remote
	// host can only run instances from repos that exist on that machine, so an
	// entry that was registered but has since been removed on the remote is
	// hidden rather than offered (and rejected at submit). Local is instant;
	// remote fans out one `ssh host git -C <path> ...` per repo concurrently.
	// The selector starts with the host-scoped list and is narrowed when the
	// result lands — local users see no flicker, remote users see the
...
func (m *home) handleRepoSelectState(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	// against the chosen host so it reappears next time. Best-effort: a failure
	// here does not block instance creation.
	if m.repoSelector.IsFreePath() && m.repoRegistry != nil {
		_ = m.repoRegistry.Add(selected, h.Name())
	}
	// MRU: move the selected repo to the head of the registry (for its host) so
	// it is offered at the top next time. Best-effort.
	if m.repoRegistry != nil {
		_ = m.repoRegistry.Touch(selected, h.Name())
	}
...
// selector, scoped to the host the selector was opened for. Silent and
// best-effort.
func (m *home) applyRepoDeletions() {
	...
	for _, path := range m.repoSelector.TakeDeletedValues() {
		_ = m.repoRegistry.Remove(path, h.Name())
	}
}
