...
// WantedBy=default.target (starts at login), and ExecStart = `cs2 daemon run`.
func TestRenderUnit_ContainsRequiredKeys(t *testing.T) {
	content := renderUnit("/usr/local/bin/cs2", "/tmp/out.log", "/tmp/err.log")

	assert.Contains(t, content, "ExecStart=/usr/local/bin/cs2 daemon run")
