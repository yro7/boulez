...
func (e ErrServiceUnsupported) Error() string {
	return "boulez daemon: no supported service manager (launchd/systemd) on this platform; use `nohup boulez daemon run &` as a dev fallback"
}

// Install installs the daemon as an OS service (launchd on macOS, systemd user
// unit on Linux). The service runs `boulez daemon run` and is kept alive by the
...
// exePath returns the absolute path to the running boulez binary. The service
// unit invokes this directly so the service always runs the same binary the
// user installed from.
func exePath() (string, error) {
	p, err := os.Executable()
	if err != nil {
		return "", fmt.Errorf("resolve boulez executable path: %w", err)
	}
...
// under the boulez config dir so it is co-located with daemon.pid/daemon.log
// and `boulez daemon status` already points the user at that area.
